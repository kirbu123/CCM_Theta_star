...
Также **(только для ROS2)** для корректной работы публикации топиков (точнее интерфейса, который её обеспечивает), нужен специальный конфиг. Это конфиг лежит в [/config/ros2/fastdds.xml](/config/ros2/fastdds.xml).
...
Для запуска сценариев в среде МФТИ - скачать [файл](https://disk.yandex.ru/d/0FIz_1jElgpq1w) и разместить там, где будет удобно.
...
По сути - описан в [QuickStart](#quickstart) - сценарий без среды, просто робот подъезжает к кубику и пробует поднять его и переложить на другое место.

### Demo with sensors and ROS2 publishing

Здесь тот же сценарий, что и первый, но здесь робот движется в среде (Warehouse - одна из готовых сред IsaccSim), а также публикует данные в ROS2 (в связи с чем требует чуть больше настройки и за счет загрузки среды сам симулятор грузится дольше).

*Note:* Требует выполненного pre-setup шага для лидара: [#LiDAR for Husky](#lidar-for-husky)
...
Уже более сложный и долгий сценарий, работает совсем не идеально (робот цепляет стены из-за чего едет супер-медленно), на данный момент в стадии доработки, но как-то работает.

*Note:* Требует выполненных обоих pre-setup шагов: [#Pre-setup for Scenarios](#pre-setup-for-scenarios). И вспомнить куда вы скачали файл среды (`path-to-MIPT-env`).
...
*Предполагается, что установка ROS2 уже выполнена и конфиг [fastdds.xml](/config/ros2/fastdds.xml) положен на место. Если нет - см. п.* [Pre-requisites or Dependencies](#pre-requisites-or-dependencies).
...
**В любых других терминалах:**
где будете запускать `rviz2` и любые другие утилиты ROS2 - просто как обычно
...
**Note:** 
Для rviz2 есть конфиг, в котором настроено отображение дерева трансформаций, камеры и тд: [config/ros2/mipt_env_rviz2_cfg.rviz](config/ros2/mipt_env_rviz2_cfg.rviz).
...
В сценарии "super plain demo" (или QuickStart) используется конфиг [config/general/default.yaml](config/general/default.yaml), его не надо специально указывать, он подгрузится автоматически.
...
Соответственно, все новые конфиги должны находится в директории [config/general](config/general), чтобы их было видно.
Это не очень гибко, но в виду сложного расположения самого склонированного репозитория - так проще.
...
Также в командной строке можно передавать любые параметры указанные в yaml конфиге, сам конфиг останется неизменным, но параметры текущего запуска будут обновлены в соответствии с переданными значениями.
...
то уровень вложенности при указании аргумента отмечается символом '.' (точка):
Например:
```bash
...
- На данный момент в модели UR5 присутствует лишний линк с названием `world`, который конфликтует с глобальным и при публикации TF трансформаций отображается строго в центре координат (фиксированно). Остальные joint -ы публикуются корректно.


## TODO

- Исправить модель кинематики UR5, так чтобы не было конфликтов с `world` и все joint-ы публиковались корректно.
