...
class MPickPlaceController(BaseController):
    """ 
    ...
    def forward(
        ...
        if self._event == 2:
            ...
            if self._event == 4.5:
                interpolated_xy = self._get_interpolated_xy(trans_x, trans_y, self._current_target_x, self._current_target_y) 
            else:   
                interpolated_xy = self._get_interpolated_xy(trans_x, trans_y, placing_position[0], placing_position[1]) 
