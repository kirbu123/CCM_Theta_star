...
from communication_msgs.msg import PickupObjectAction, PickupObjectGoal
from communication_msgs.msg import PutObjectAction, PutObjectGoal
from communication_msgs.msg import MoveToAction, MoveToGoal, MoveToResult, MoveToFeedback
...
class MoveToActionServer:
    ...
    def execute(self, goal: MoveToGoal):
        ...
        while self.result_queue.empty():
            ...
            rospy.loginfo(f'[{self.server_name}] Sending feedback:', feedback.__str__())
    ...
    def execute(self, goal: PickupObjectGoal):
        ...
        while self.result_queue.empty():
            ...
            if self.server.is_preempt_requested():
                ...
                self.server.set_preempted()
                return
            rospy.loginfo(f'[{self.server_name}] Waiting for result...')
            # feedback = self.feedback_queue.get()
            # self.server.publish_feedback(feedback)
            # self.feedback_queue.task_done()
...
class OpenSeedActionServer:
    def __init__(self, 
        ...
        self.server = actionlib.SimpleActionServer("text_query_generation", OpenSeeDSetterAction, self.execute, False)
        self.server.start()
        self.task_queue = task_queue
    ...
    def execute(self, goal: OpenSeeDSetterGoal):
        ...
        # result = self.result_queue.get()
        rospy.loginfo(f'[{self.server_name}] Done!')
