...
while simulation_app.is_running():
    ...
    if my_world.is_playing():
        # ##########################################   Navi One   ########################
        if (
            navigation_one_start
            and (not navigation_one_finish)
            and (not pick_up_start)
            and (not pick_up_finish)
            and (not place_start)
            and (not place_finish)
        ):
            print("Step navi one!\n")
            # ###################################### Init UR5 controller ################################
            ...
            for i, milestone in enumerate(cfg.husky_pick_position):
                ...
                while not (distance < 1.2):
                    ...
                    my_world.step(render=True)
                    distance = np.sum(
                        (husky.get_world_pose()[0][:2] - husky_pick_position) ** 2
            ...
            pick_place_controller._cspace_controller.reset(
                cube
            )  # Very Important! Here init the RMPFlow's pose after husky move!!!!

        # #################################   To pick up   #############################

        if (
            (navigation_one_finish)
            and (pick_up_start)
            and (not pick_up_finish)
            and (not place_start)
            and (not place_finish)
            ...
            # print("Step pick up!\n")
            wheel_actions = husky_controller.forward(
                start_position=navi_one_end_position,
        ...
        if (
            (navigation_one_finish)
            and (pick_up_finish)
            and (navigation_two_start)
            and (not navigation_two_finish)
            and (not place_start)
        ):
            print("Step navi 2!\n")
            ...
            for i, milestone in enumerate(cfg.husky_place_position):
                ...
                while not (distance < 1.2):
                    ...
                    my_world.step(render=True)
                    distance = np.sum(
                        (husky.get_world_pose()[0][:2] - milestone) ** 2
                    )  # Compute distance between husky and target
        ...
        if (
            (navigation_one_finish)
            and (pick_up_finish)
            and (navigation_two_finish)
            and (place_start)
            and (not place_finish)
        ):
            wheel_actions = husky_controller.forward(
                start_position=navi_two_end_position,
            ...
            actions = pick_place_controller.forward(
                ...
                # ## the picking position is not matter here, because only pick
                picking_position=cube.get_world_pose()[0],
